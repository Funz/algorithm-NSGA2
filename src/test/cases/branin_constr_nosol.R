...
f <- function(x) {
    ...
    , x[2]-1) # : constraint x2>1 -> NEVER ACHIEVED
}
input.f = list(
    x1=list(min=0,max=1),
    x2=list(min=0,max=1)
...
test = function(algorithm_file) {
    ...
    if (!isTRUE(test_that("branin min",{expect_null(results$min)}))) quit(status=1) # should not return any solution !
}
