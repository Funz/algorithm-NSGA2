...
test = function(algorithm_file) {
    results = run.algorithm(algorithm_file, options=list(generations='10'),fun=list(input=input.f,output=output.f,fun=f))
    if (!isTRUE(test_that("branin_-510_015 min",{expect_equal(min(as.numeric(results$min)),min.f,tolerance = .01)}))) quit(status=1)
}
